...
unconfirmed if the running server documents different flag names. **Stop
and tell the user to check if you cannot confirm the write tools are off.
Do not continue into Step 1 on an unconfirmed configuration.**
...
one bounded per-bill fan-out capped at 50 calls total.** Nothing loops per
transaction; nothing re-pulls a report at the top level. Pagination inside
a single call stops at 20 pages — tell the user the period has too much
activity for one pass rather than paging indefinitely or treating a
partial page as the whole result. The 50-call fan-out covers MCP builds
that expose payments/credits only by linked bill ID; above that cap, or
with no bounded alternative, use the date-range fallback in
`references/pull-recipes.md`, or stop and tell the user the period is too
large for this MCP's payment interface.
...
- **Aging population**: every vendor bill still open, unpaid, or partially
  ...
  for Step 4's AP Aging bucket view. A bill dated this period and already
  fully paid does not belong here; it has no open balance to age.
...
the Bills-only scope, all five entity pulls, the bucketing and
multicurrency rules, the empty-pull cross-checks, and pull-failure
...
population to ask documents for, the flag-and-exclude vs.
flag-and-still-match split, and the field-comparison rule for spotting a
duplicate.
...
ambiguity), the stable processing order, how the payable total is computed
from each bill's open balance, the two narrow exclusions, how a vendor
credit's unapplied portion nets in, and the exclusion-aware cross-check
against QBO's own AP Aging report. Build the aging bucket view described
...
1. The resolved period, with the Purchase/Expense out-of-scope note where
   that applies.
2. The stop-and-report, if any Step 2 pull failed or was unusable —
   replacing the rest of the sequence.
...
stated up front. See `references/scope-boundaries.md` for the full list.
